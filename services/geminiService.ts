...
export const generateCharacterSprites = async (
  ...
  const animationPrompt = isBoxer ? `
    ...
    Row 1: IDLE (Breathing/Bobbing) - 4 frames:
    - Character in boxing guard. Head bobs up (frame 1) and down (frame 4).
    Row 2: WALK (Footwork) - 4 frames:
    - Walking cycle with gloves up in guard. Clear alternating leg movement.
    Row 3: PUNCH (Attack) - 4 frames:
    - Wind up, half-extension, full impact (straight punch), and recovery.
  ` : `
