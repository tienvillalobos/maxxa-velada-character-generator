...
import { AppStatus, SpriteResult, GeneratorType, ANIMATION_SETS } from './types';
  ...
  const downloadZip = async () => {
    ...
    try {
      ...
      for (let r = 0; r < rows; r++) {
        const folder = zip.folder(animations[r]);
        for (let c = 0; c < cols; c++) {
          ...
          if (blob && folder) {
            folder.file(`${animations[r]}_${c}.png`, blob);
          }
        }
      }
      ...
      link.download = `pixel_${result.type.toLowerCase()}_animations.zip`;
            ...
            <div className="bg-slate-900 border-4 border-slate-800 rounded-3xl p-8 overflow-hidden">
              ...
              <div className="mt-6 flex flex-wrap justify-center gap-4 text-slate-500 text-[9px] font-black uppercase tracking-[0.2em] pixel-font text-center">
                {ANIMATION_SETS[result.type].map((anim, idx) => (
                  <span key={anim} className="bg-slate-800 px-3 py-1 rounded">Fila {idx+1}: {anim}</span>
                ))}
              </div>
            </div>
              ...
              <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    {result.type === 'BOXER' ? 'idle/, walk/, punch/' : 'idle/, cheer/, surprised/'} - Cada una con sus frames .png listos para tu juego.
                  </p>
                </div>
              </div>
              <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 flex items-start gap-4">
